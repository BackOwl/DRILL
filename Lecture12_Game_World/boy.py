...
SHIFT_DOWN, SHIFT_UP, DASH_TIMER, SHIFT_DEBUG, SPACE = range(10)

event_name = ['RIGHT_DOWN', 'LEFT_DOWN', 'RIGHT_UP', 'LEFT_UP', 'SLEEP_TIMER', \
'SHIFT_DOWN', 'SHIFT_UP', 'DASH_TIMER','SHIFT_DEBUG','SPACE' ]
...
key_event_table = {
    ...
    (SDL_KEYDOWN, SDLK_d): SHIFT_DEBUG,
...
next_state_table = {
    ...
    SleepState: {LEFT_DOWN: RunState, RIGHT_DOWN: RunState, LEFT_UP: RunState, RIGHT_UP: RunState,
                 SPACE: IdleState},
    DashState: {SHIFT_UP: RunState, SHIFT_DOWN: DashState, DASH_TIMER: RunState,
                RIGHT_UP: IdleState, LEFT_UP: IdleState, LEFT_DOWN: IdleState, RIGHT_DOWN: IdleState,SPACE:RunState}
}
...
class Boy:
    ...
    def update(self):
        ...
        if len(self.event_que) > 0:
            ...
            #오류찾기
            try:#우선 시도해본다
                history.append((self.cur_state.__name__, event_name[event]))#로그기록(내 조작기록)기록
                self.cur_state.exit(self, event)
                self.cur_state = next_state_table[self.cur_state][event]
            except:#문제가 발생했을경우
                print('cur state: ',self.cur_state.__name__,'\nevent: ',event_name[event])
                exit(-1)
